...
DESCRIPTION
    Check bitpool supported by all A2DP devices during a media channel reopen
    to make sure a bitpool is chosen that falls within the supported limits.

*/
static void checkSbcBitpool(uint8 *min_bitpool, uint8 *max_bitpool)
{
    
#ifdef DUAL_STREAM    
    devInstanceTaskData *inst;
    uint16 i;
    uint8 chosen_min_bp = *min_bitpool;
    uint8 chosen_max_bp = *max_bitpool;    
    a2dpAudioQuality audio_quality = A2DP_AUDIO_QUALITY_UNKNOWN;
    uint16 number_a2dp_devices = 0;
    ...
    for (i = 0; i < MAX_NUM_DEV_CONNECTIONS; i++)
    {
        ...
        if (inst != NULL)
        {                  
            if ((inst->sbc_min_bitpool != 0) && (inst->sbc_min_bitpool > chosen_min_bp))
                chosen_min_bp = inst->sbc_min_bitpool;
            if ((inst->sbc_max_bitpool != 0) && (inst->sbc_max_bitpool < chosen_max_bp))
                chosen_max_bp = inst->sbc_max_bitpool;
            
            if (inst->a2dp_audio_quality < audio_quality)
            {
                audio_quality = inst->a2dp_audio_quality;                
            }
            if (inst->a2dp_sig_sink || inst->a2dp_reopen)
            {
                number_a2dp_devices++;
            }
        }
    ...
    if (number_a2dp_devices == MAX_NUM_DEV_CONNECTIONS)
    {
        uint16 temp_max_bitpool = max_bitpool_array[audio_quality];
        if (temp_max_bitpool < chosen_max_bp)
            chosen_max_bp = temp_max_bitpool;
    }
    
    *min_bitpool = chosen_min_bp;
    *max_bitpool = chosen_max_bp;
#endif    
    
    DEBUG_A2DP(("    chosen bitpool min[0x%x] max[0x%x]\n",*min_bitpool,*max_bitpool));    
}
        ...
        case A2dpStateOpening:
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ((( configureIsPinRequested(inst) && !configureIsPinListExhausted(inst) ) ||
                    ...
                    else if( msg->status == a2dp_key_missing )
                    {
        ...
        {          
            {
                ...
                if ((the_app->a2dp_source == SourceAnalog) && !a2dp_streaming)
                {
                    /* Load the Kap file and let DSP run */
                    a2dpStreamStartDsp(TRUE);
                }
...
#ifdef DUAL_STREAM        
    else if (a2dpSlcIsDifferentMediaStreaming(inst) && a2dpSeidIsSbc(inst->a2dp_seid))
    {
        /* find bitpools supported by remaining device */
        uint16 max_bitpool = SBC_BITPOOL_HIGH_QUALITY;
        uint16 min_bitpool = SBC_BITPOOL_MIN;
        /* reset min and max bitpools for this device */
        inst->sbc_min_bitpool = SBC_BITPOOL_MIN;
        inst->sbc_max_bitpool = SBC_BITPOOL_HIGH_QUALITY; 
        getMinAndMaxBitpools(&min_bitpool, &max_bitpool);
        /* send new bitpool to DSP */
        if (max_bitpool != the_app->a2dp_data.codecData.bitpool)
        {
            DEBUG_A2DP(("    set new bitpool [%d]\n", max_bitpool));
            KalimbaSendMessage(KALIMBA_SET_BITPOOL_MESSAGE, max_bitpool, 0, 0, 0);
            KalimbaSendMessage(KALIMBA_SET_POOR_LINK_BITPOOL_MESSAGE, max_bitpool, 0, 0, 0);
            the_app->a2dp_data.codecData.bitpool = max_bitpool;
        }
    }
#endif
}
...
static bool handleConfigCodec(A2DP_CONFIGURE_CODEC_IND_T *msg)
{
    if (msg->codec_service_caps[3] == AVDTP_MEDIA_CODEC_SBC)
    {
        ...
        checkSbcBitpool(&msg->codec_service_caps[6], &msg->codec_service_caps[7]);
    ...
    else if (msg->codec_service_caps[3] == AVDTP_MEDIA_CODEC_NONA2DP)
    {
        ...
        */
        ...
        {
            if (the_app->a2dp_source == SourceUsb)
                msg->codec_service_caps[11] &= 0x01;
            else
...
/****************************************************************************
  NAME
      areCodecSettingsIdentical

DESCRIPTION
    Determines if the codec settings passed into the function are compatible
    with the stored application settings for the currently active A2DP media.

*/
static bool areCodecSettingsIdentical(devInstanceTaskData *inst, A2DP_CODEC_SETTINGS_IND_T *msg)
{
    uint8 open_seid = the_app->a2dp_active_seid;
    
    if (open_seid != msg->seid)
    {
        if (a2dpSeidIsSbc(open_seid) && a2dpSeidIsSbc(msg->seid))
        {
            /* Both SBC so allowed */
        }
        else if (a2dpSeidIsFaststream(open_seid) && a2dpSeidIsFaststream(msg->seid))
        {
            /* Both FastStream so allowed */
        }
        else
        {
            DEBUG_A2DP(("seid %d %d\n",open_seid,msg->seid));
    ...
    if (the_app->a2dp_sample_rate != msg->rate)
    {
        DEBUG_A2DP(("rate %ld %ld\n",the_app->a2dp_sample_rate,msg->rate));
        return FALSE;
    }
    if (the_app->a2dp_channel_mode != msg->channel_mode)
    {
        DEBUG_A2DP(("chn mode %d %d\n",the_app->a2dp_channel_mode,msg->channel_mode));
        return FALSE;
    }
    if (the_app->a2dp_data.codecData.content_protection != msg->codecData.content_protection)
    {
        DEBUG_A2DP(("protect %d %d\n",the_app->a2dp_data.codecData.content_protection,msg->codecData.content_protection));
        return FALSE;
    }
    if (the_app->a2dp_data.codecData.voice_rate != msg->codecData.voice_rate)
    {
        DEBUG_A2DP(("voice rate %ld %ld\n",the_app->a2dp_data.codecData.voice_rate,msg->codecData.voice_rate));
        return FALSE;
    }
    if (the_app->a2dp_data.codecData.format != msg->codecData.format)
    {
        DEBUG_A2DP(("format %d %d\n",the_app->a2dp_data.codecData.format,msg->codecData.format));
        return FALSE;
    }
    if (the_app->a2dp_data.codecData.packet_size != msg->codecData.packet_size)
    {
        DEBUG_A2DP(("pck size %d %d\n",the_app->a2dp_data.codecData.packet_size,msg->codecData.packet_size));
        return FALSE;
    }
...
DESCRIPTION
    Retrieve the min and max bitpool from the supplied codec capabilities.
*/
static void getBitpoolRange(uint8 *codec_caps, uint16 size_caps, uint8 *sbc_min_bitpool, uint8 *sbc_max_bitpool)
{
    uint16 i;
    uint16 sz_caps = size_caps;
    uint8 *caps = codec_caps;
    
    DEBUG_A2DP(("A2DP configured caps:\n"));
    for (i=0; i<size_caps; i++)
    {
        DEBUG_A2DP((" %x",codec_caps[i]));    
    }
    DEBUG_A2DP(("\n"));
    ...
    *sbc_max_bitpool = 0;
    ...
    while (sz_caps != 0)
    {
        ...
        if (service == AVDTP_SERVICE_MEDIA_CODEC)
        {
            if ((losc >= 6) && ((caps)[3] == AVDTP_MEDIA_CODEC_SBC))
            {
                *sbc_min_bitpool = (caps)[6];    
                *sbc_max_bitpool = (caps)[7];        
                DEBUG_A2DP(("    stored bitpool range: min[0x%x] max[0x%x]\n",*sbc_min_bitpool,*sbc_max_bitpool));
                return;
            }
        }
    ...
    DEBUG_A2DP(("    bitpool range not found\n"));
}
#endif /* DUAL_STREAM */
...
static void handleA2DPCodecSettingsInd(devInstanceTaskData *theInst, A2DP_CODEC_SETTINGS_IND_T *msg)
{
    ...
    switch (getA2dpState(theInst))
    {
        ...
        {
#ifdef DUAL_STREAM
            if (a2dpSeidIsSbc(msg->seid))    
            {
                uint8 sbc_min_bitpool;
                uint8 sbc_max_bitpool;
                uint16 size_caps;  
                /* Get the Stream End Point configuration */
                codec_caps = A2dpGetCurrentSepConfiguration(msg->a2dp, &size_caps);
                /* Get bitpool max and min values from configuration */
                getBitpoolRange(codec_caps, size_caps, &sbc_min_bitpool, &sbc_max_bitpool);
                theInst->sbc_min_bitpool = sbc_min_bitpool;
                theInst->sbc_max_bitpool = sbc_max_bitpool;
                /* free memory */
                free(codec_caps);        
            }    
       
            /* If another media channel is already open with a different device then this
                new media channel configuration must be compared with the current one. 
                If they differ, both media channels should be closed and re-opened with
                the same configuration they can both use. */
            if (a2dpSlcIsDifferentMediaOpen(theInst))
            {
                DEBUG_A2DP(("    a2dp media already open\n"));

                if (!areCodecSettingsIdentical(theInst, msg))
                {
                    uint16 i;
                    DEBUG_A2DP(("   - configurations are different so close media\n"));
                    /* The streams are incompatible with each other, so must close all streams and reopen with SBC */
                    theInst->a2dp_closing = TRUE;            
                    for (i = 0; i < MAX_NUM_DEV_CONNECTIONS; i++)
                    {
                        if (the_app->dev_inst[i] != NULL)
                        {
                            switch (the_app->dev_inst[i]->a2dp_state)
                            {
                                ...
                                {                        
                                    ...
                                    the_app->dev_inst[i]->a2dp_reopen_codec = 0;
            ...
            if(the_app->a2dp_source == SourceAnalog)
            {
                ...
                config.adc_sample_rate = codec_sampling_frequency;
                   ...
                   if (msg->rate == 44100)
                    the_app->a2dp_sample_rate  = 44118;
            #endif
            ...
            the_app->a2dp_data.codecData.source_type        = the_app->a2dp_source;
