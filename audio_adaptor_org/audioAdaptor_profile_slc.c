...
void profileSlcConnectReq (void)
{
    ...
    {       
        ...
        case AppStateConnecting:    
        {
            ...
            /* Read info from stored paired device data */
            if (!inst->paired_list_read && configureGetDeviceInfo (&inst->bd_addr, &pin_idx, &remote_profiles, &pin_authorised))
            {
                ...
                DEBUG_CONN(("Read paired device details pin_idx[%d] start_idx[%d] auth[%d] profiles[0x%x]\n",pin_idx, pin_idx, pin_authorised, inst->remote_profiles));
            }
            inst->paired_list_read = TRUE;
            
            /* See if SDP search data exists */
            if (!BdaddrIsZero(&the_app->search_bdaddr) && BdaddrIsSame(&the_app->connect_bdaddr, &the_app->search_bdaddr))
            {
                inst->remote_profiles |= the_app->remote_profiles;
            }
            ...
            if ( (the_app->supported_profiles & (ProfileAghsp | ProfileAghfp)) && 
                 (inst->remote_profiles & (ProfileAghsp | ProfileAghfp)) && 
                 !(inst->responding_profiles & (ProfileAghsp | ProfileAghfp)) )
            {
                if (aghfpSlcConnect(inst))
                {
                    ...
                    the_app->connecting_profile = (the_app->supported_profiles & (ProfileAghsp | ProfileAghfp));
                    break;
                }            
            }
            
            if ( (the_app->supported_profiles & ProfileA2dp) && 
                 (inst->remote_profiles & ProfileA2dp) && 
            ...
            if ( (the_app->supported_profiles & ProfileAvrcp) && 
                 (inst->remote_profiles & ProfileAvrcp)  && 
        ...
        switch ( the_app->app_state )
        {
            ...
            default:
            {
                ...
                else
                {
                    ...
                    if ( the_app->auto_connect_in_progress )
                    {
                        if ( configureGetBdaddrNextRecentDevice(&the_app->connect_bdaddr) )
                        {    /* Attempt to connect to next most recent device */
                            kickCommAction(CommActionConnect);
                        }
                        else
    ...
    {    /* Successfully connected to one or more available profiles */
        ...
        switch ( the_app->app_state )
        {
            ...
            case AppStateConnecting:
            {
                ...
                configureStoreCurrentPairedDevice(inst);
                if ( the_app->auto_connect_in_progress )
                {
                    the_app->auto_connect_in_progress = FALSE;
                }
...
void profileSlcConnectCfm (devInstanceTaskData *inst, mvdProfiles profile, bool remote_connecting)
{
   ...
   stopAppProfileConnectTimer( inst );    
    ...
    if ( connected_profiles==ProfileNone && (all_responded || configureIsPinListExhausted(inst)) ) 
...
void profileSlcDisconnectInd (devInstanceTaskData *inst, mvdProfiles profile)
{
    ...
    switch (profile)
    {
        ...
        case ProfileAghfp:
        {
            usbHidIssueHidCommand(AppEventHfpLinkDisconnected);
            break;
        }
        case ProfileA2dp:
        {
            usbHidIssueHidCommand(AppEventA2dpLinkDisconnected);
        ...
        case ProfileAvrcp:
        {
            usbHidIssueHidCommand(AppEventAvrcpLinkDisconnected);
            break;
        }
        default:
        {
...
/*#if 0    */
    ...
    {    /* Attempt to find a suitable device and connect to it */
        ConnectionSmDeleteAllAuthDevices(0);  /* Ensure CLs link key list is erased */
        scanMakeDiscoverable();
