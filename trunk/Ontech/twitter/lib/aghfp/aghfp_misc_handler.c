...
void aghfpHandleKeypadCommand(AGHFP *aghfp, uint16 keycode)
{
	if (keycode==200)
	{
		...
		{	
			...
			{
				/* Inform app */
				MAKE_AGHFP_MESSAGE(AGHFP_HS_CALL_ANSWER_IND);  /* This is really acting as a template for all possible AGHFP_HS primitives */
				message->aghfp = aghfp;
...
void AghfpSendSubscriberNumber(AGHFP *aghfp, aghfp_subscriber_info *sub)
{
    if ( supportedProfileIsHfp15(aghfp->supported_profile) )
    {
		...
		if ( sub->size_number )
		{
    	    memcpy(message->number, sub->number, sub->size_number);
	    }
	    else
	    {
    	    *message->number = 0;
...
void AghfpSendCurrentCall(AGHFP *aghfp, aghfp_call_info *call)
{
    if ( supportedProfileIsHfp15(aghfp->supported_profile) )
    {
		...
		if ( call->size_number )
		{
    	    memcpy(message->number, call->number, call->size_number);
	    }
	    else
	    {
    	    *message->number = 0;
...
{
    if ( supportedProfileIsHfp15(aghfp->supported_profile) && mode==3 && format==0 )
    {
    	aghfpSendOk(aghfp);
    }
    else
...
void AghfpSendNetworkOperator(AGHFP *aghfp, uint8 mode, uint16 size_operator, uint8 *operator)
{
	if ( supportedProfileIsHfp15(aghfp->supported_profile) )
	{
		...
		memcpy(message->operator, operator, size_operator);
