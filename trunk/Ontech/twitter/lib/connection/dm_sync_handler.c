...
static void sendSyncConnectCfmToClient(Task appTask, Task clientTask, hci_status status, const link_params *params)
{
    if (clientTask)
    {
...
void connectionHandleSyncConnectReq(const CL_INTERNAL_SYNC_CONNECT_REQ_T *req)
{
	...
	*/	
    if ( !SinkGetBdAddr(req->sink, &addr) )
    {
        ...
        sendSyncConnectCfmToClient(0, req->theAppTask, hci_error_no_connection, 0);
    }
    else
    {
        ...
        prim->tx_bdw = req->config_params.tx_bandwidth;
        prim->rx_bdw = req->config_params.rx_bandwidth;
        prim->max_latency = req->config_params.max_latency;
        prim->voice_settings = req->config_params.voice_settings;
        prim->retx_effort = req->config_params.retx_effort;
        prim->packet_type = (hci_pkt_type_t)req->config_params.packet_type;
...
RETURNS
	void
*/
void connectionHandleSyncConnectCfm(Task theAppTask, const DM_EX_SYNC_CONNECT_CFM_T *cfm)
{
    bdaddr addr;
    ...
    params.sco_handle = cfm->lm_sco_handle;
    params.sink = cfm->sink;

	/* Tell the client task that we have a cfm for the Sync request */
	sendSyncConnectCfmToClient(theAppTask, (Task) ((uint16)cfm->pv_cbarg), cfm->status, &params);
...
RETURNS
	void
*/
void connectionHandleSyncConnectCompleteInd(Task theAppTask, const DM_EX_SYNC_CONNECT_COMPLETE_IND_T *ind)
{
    bdaddr addr;
    ...
    params.sco_handle = ind->lm_sco_handle;
    params.sink = ind->sink;

	/* Tell the client task that we have a cfm for the Sync request */
	sendSyncConnectCfmToClient(theAppTask, (Task) ((uint16)ind->pv_cbarg), ind->status, &params);
...
{
    MAKE_PRIM_T(DM_SYNC_CONNECT_RES);
    
    connectionConvertBdaddr_t(&prim->bd_addr, &res->bd_addr);
    
    prim->tx_bdw = res->config_params.tx_bandwidth;
    prim->rx_bdw = res->config_params.rx_bandwidth;
    prim->max_latency = res->config_params.max_latency;
    prim->voice_settings = res->config_params.voice_settings;
    prim->retx_effort = res->config_params.retx_effort;
    prim->packet_type = (hci_pkt_type_t)res->config_params.packet_type;
    
    /* EDR bits use inverted logic at HCI interface */
    /*prim->packet_type ^= sync_all_edr_esco;*/
    
    prim->accept = (uint16)res->response;
    
    if ( res->response )
    {
        prim->reason = hci_success;
    }
    else
    {
        /* Reason must be one of:  hci_error_rej_by_remote_no_res,  hci_error_rej_by_remote_sec  */ 
        /* or  hci_error_rej_by_remote_pers.                                                     */
        prim->reason = hci_error_rej_by_remote_no_res;
    }
...
void connectionHandleSyncDisconnectReq(const CL_INTERNAL_SYNC_DISCONNECT_REQ_T *req)
{
    ...
    MAKE_PRIM_T(DM_EX_SYNC_DISCONNECT_REQ);
    prim->sink = req->audio_sink;
...
RETURNS
	void
*/
void connectionHandleSyncDisconnectInd(const DM_EX_SYNC_DISCONNECT_IND_T *ind)
{
    bdaddr addr;
...
RETURNS
	void
*/
void connectionHandleSyncDisconnectCfm(const DM_EX_SYNC_DISCONNECT_CFM_T *cfm)
{
    bdaddr addr;
...
void connectionHandleSyncRenegotiateReq(const CL_INTERNAL_SYNC_RENEGOTIATE_REQ_T *req)
{
    ...
    {
        MAKE_PRIM_T(DM_EX_SYNC_RENEGOTIATE_REQ);
        prim->sink = req->audio_sink;
        prim->max_latency = req->config_params.max_latency;
        prim->retx_effort = req->config_params.retx_effort;
        prim->packet_type = (hci_pkt_type_t)req->config_params.packet_type;
...
RETURNS
	void
*/
void connectionHandleSyncRenegotiateInd(const DM_EX_SYNC_RENEGOTIATE_IND_T *ind)
{
    if (ind->pv_cbarg)
    {
...
RETURNS
	void
*/
void connectionHandleSyncRenegotiateCfm(const DM_EX_SYNC_RENEGOTIATE_CFM_T *cfm)
{
    if (cfm->pv_cbarg)
    {
        MAKE_CL_MESSAGE(CL_DM_SYNC_RENEGOTIATE_IND);
