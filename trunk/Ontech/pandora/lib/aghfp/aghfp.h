...
typedef enum
{
    ...
    aghfp_connect_key_missing                   /*!< Unsuccessful due to missing link key on remote device*/
...
/*!
	@brief WBS codec type
*/
typedef enum
{
	aghfp_wbs_codec_cvsd,
	aghfp_wbs_codec_sbc,
	aghfp_wbs_codec_amr,
	aghfp_wbs_codec_evrc,
	aghfp_wbs_codec_other
} aghfp_wbs_codec;
...
/*! @brief Define the types for the upstream messages sent from
	the Aghfp profile library to the application.
*/
#define AGHFP_MESSAGE_BASE	0x6300
...
/* 3GPP SMS */
	AGHFP_READ_MESSAGE_IND,
	AGHFP_NEW_MESSAGE_IND,
	AGHFP_SEND_MESSAGE_IND,
...
typedef struct
{
	AGHFP 			*aghfp;
	uint16			index;
} AGHFP_READ_MESSAGE_IND_T;

typedef struct
{
	AGHFP 			*aghfp;
	uint16			mode;
}AGHFP_NEW_MESSAGE_IND_T;
...
/*!	@brief Initialise an instance of the AGHFP library.
	...
	@param aghfp_supported_profile Profile supported. Choose from the members
	       of the aghfp_profile enum - either aghfp_headset_profile or
	       aghfp_handsfree_profile.
...
/*!	@brief Respond to an AGHFP_SLC_CONNECT_IND message from the HF.
	...
	The Bluetooth address of the device that initiated to the Service Level Connection
    is passed in using bd_addr. This should be the same as the address in the
	AGHFP_SLC_CONNECT_IND message.

	The application will receive an AGHFP_SLC_CONNECT_CFM message from the library
	to indicate the success or otherwise of the connection attempt. */
void AghfpSlcConnectResponse(AGHFP *aghfp, bool response, const bdaddr *bd_addr);
