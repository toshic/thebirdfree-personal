...
NAME 
	hidL2capConfigure
DESCRIPTION
	Initialises a L2CAP configuration structure.
RETURNS
	void
*/
void hidL2capConfigure(HID *hid, l2cap_config_params *l2cap_config, int psm)
{
	/* Initialise L2CAP configuration structure for control or interrupt channel */
    l2cap_config->mtu_local = HID_L2CAP_MTU;
	...
	l2cap_config->qos.service_type = (psm == HID_PSM_CONTROL) ? DEFAULT_SERVICE_TYPE : ((hid->config->qos_guaranteed) ? SERVICE_GUARANTEED : DEFAULT_SERVICE_TYPE);
	...
	l2cap_config->qos.latency = (psm == HID_PSM_CONTROL) ? DEFAULT_LATENCY : hid->config->latency;
    l2cap_config->qos.delay_var = DEFAULT_DELAY_VAR;
    l2cap_config->accept_qos_settings = 1;
    l2cap_config->timeout = HID_L2CAP_TIMEOUT;
}
...
DESCRIPTION
	...
	Sends a HID_DISCONNECT_IND message to the application.
RETURNS
	void
*/
...
static void hidDisconnectedEnter(HID *hid)
{
	...
	assert(hidConnIsDisconnected(hid, HID_PSM_CONTROL));
	assert(hidConnIsDisconnected(hid, HID_PSM_INTERRUPT));
