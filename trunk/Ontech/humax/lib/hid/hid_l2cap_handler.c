...
{ 
	...
	else
	{
		...
		ConnectionL2capConnectResponse(&hid_lib->task, FALSE, ind->psm, ind->connection_id, 0);            
	}
}
...
{ 
   	...
   	if (cfm->status == l2cap_connect_success)
    {
		...
		else if (hidConnIsConnected(hid, HID_PSM_CONTROL) && hidConnIsDisconnected(hid, HID_PSM_INTERRUPT))
		{
			...
			/* Start outgoing interrupt connection */
			hidL2capConfigure(hid, &l2cap_config, HID_PSM_INTERRUPT);
		    hidConnConnecting(hid, HID_PSM_INTERRUPT, 0);
			ConnectionL2capConnectRequest(&hid->task, &hid->remote_addr, HID_PSM_INTERRUPT, HID_PSM_INTERRUPT, &l2cap_config);
		}
    }    
    else
	{
