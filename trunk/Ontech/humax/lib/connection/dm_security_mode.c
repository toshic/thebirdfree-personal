...
void ConnectionSmSetSecurityMode(Task theAppTask, dm_security_mode sec_mode, encryption_mode enc_mode)
{
...

    {
		...
		MessageSendConditionallyOnTask(connectionGetCmTask(), CL_INTERNAL_SM_SET_SC_MODE_REQ, message, &theCm.smState.setSecurityModeLock);
    }
}
...
void ConnectionSmSecModeConfig(Task theAppTask, cl_sm_wae write_auth_enable, bool debug_keys, bool legacy_auto_pair_key_missing)
{
...
#endif
	{
	    ...
	    MessageSendConditionallyOnTask(connectionGetCmTask(), CL_INTERNAL_SM_SEC_MODE_CONFIG_REQ, message, &theCm.smState.setSecurityModeLock);
	}
}
