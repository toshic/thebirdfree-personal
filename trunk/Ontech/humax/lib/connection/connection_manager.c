        ...
        if(curr->cid == cid)
        {
			...
			else
			{
				if (i != (multiple_channels_entries - 1))
					memmove(multiple_channels_list_head + i, multiple_channels_list_head + i + 1, sizeof(multiple_channels_instance) * (multiple_channels_entries-i-1)); 
