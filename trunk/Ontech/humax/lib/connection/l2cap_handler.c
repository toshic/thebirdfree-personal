...
static void deleteConnection(uint16 psm, cid_t cid)
{
	CREATE_L2CAP_CONN_ID(psm, cid);
	(void) connectionDeleteInstance(conn_l2cap, new_id);
}


/* Retrieve a connection state for the linked list */
static conn_instance *getConnection(uint16 psm, cid_t cid)
{
	CREATE_L2CAP_CONN_ID(psm, cid);
	return connectionGetInstance(conn_l2cap, new_id);
}
...
{	
	conn_instance *this_conn = getConnection(psm, cid);
...
{
	conn_instance *this_conn = getConnection(L2CA_PSM_INVALID, cid);
	...
	if (this_conn)
	{
		...
		*/
		...
		deleteConnection(this_conn->id.l2cap_id.psm, cid);
	}
	else
	{
		/* 
...
void connectionHandleL2capConnectCfm(const L2CA_CONNECT_CFM_T *cfm)
{
	...
	if (cfm->response == L2CA_CONNECTION_PENDING)
		this_conn = getConnection(cfm->psm, L2CA_CID_INVALID);
	else
	{
		/* Look for the cid in the connect_cfm */
		this_conn = getConnection(cfm->psm, cfm->cid);
	...
	else
	{
		...
		else 
		{
			...
			deleteConnection(cfm->psm, cfm->cid);			
		}
	}
}
...
{
	conn_instance *this_conn = getConnection(L2CA_PSM_INVALID, cfm->cid);
...
void connectionHandleL2capConfigInd(L2CA_CONFIG_IND_T *ind)
{
	...
	*/
	...
	conn_instance *this_conn = getConnection(L2CA_PSM_INVALID, ind->cid);
...
void connectionHandleL2capConnectInd(const L2CA_CONNECT_IND_T *ind)
{
    ...
    conn_instance *this_conn = getConnection(ind->psm, ind->cid);
...
{
	/* Get the connection state */
	conn_instance *this_conn = getConnection(res->psm_local, res->cid);
	...
	if (this_conn)
	{
		...
		else
		{
			...
			/* Delete the connection state */
			deleteConnection(res->psm_local, this_conn->id.l2cap_id.cid);
		}
	}
}
