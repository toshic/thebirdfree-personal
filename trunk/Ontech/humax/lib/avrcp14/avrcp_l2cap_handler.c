...
*/
void avrcpHandleL2capRegisterCfm(AVRCP *avrcp,
...
DESCRIPTION
    ...
    connect request.
*/
void avrcpHandleL2capConnectIndReject(  AVRCP   *avrcp,
                   const CL_L2CAP_CONNECT_IND_T *ind)
{
    /* Send a connect response rejecting the connection. */
    ConnectionL2capConnectResponse(&avrcp->task, 0, 
                                    ind->psm, ind->connection_id, 0);
}
