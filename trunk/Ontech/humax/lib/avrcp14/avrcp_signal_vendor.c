                                 ...
                                 const AVRCP_INTERNAL_VENDORDEPENDENT_REQ_T *req)
{
    ...
    /* Check whether it is a Meta Data Command. For Meta Data Commands, avrcp 
       fills the identifier before calling it */
    if (avrcp->identifier)
    {
        /* Extract the extra data information which is not passed in
             data parameter */
        extra_data =  METADATA_HEADER_SIZE + 
                        (((avrcp->identifier[METADATA_LEN_OFFSET] << 8) | 
                          avrcp->identifier[METADATA_LEN_OFFSET+1]) - 
                          req->data_length);

        header_size += extra_data;
        packet_size += extra_data;
    }
        ...
        if(packet_size < SinkSlack(avrcp->sink))
        {
            ...
            if(avrcp->identifier)
            {
                memmove(&ptr[avctp_header_size+AVRCP_VENDOR_HEADER_SIZE], 
                        avrcp->identifier, extra_data);

                /* Free the Data*/
                free(avrcp->identifier);
                avrcp->identifier=NULL;
            }
...
void avrcpHandleVendorResponse(AVRCP *avrcp, const uint8 *ptr, uint16 packet_size)
{
    if(avrcp->pending == avrcp_vendor)
    {
        ...
        {
            /* Set a watchdog timeout start waiting for final response*/
            MessageSendLater( &avrcp->task, 
                              AVRCP_INTERNAL_WATCHDOG_TIMEOUT, 
                              0, AVCTP_WATCHDOG_TIMEOUT );
    ...
    else
    {
        ...
        {
            /* Received invalid packet size - Cannot do anything here. */

            /* No longer waiting */
            (void) MessageCancelAll(&avrcp->task, 
                                    AVRCP_INTERNAL_WATCHDOG_TIMEOUT);
            avrcp->pending = avrcp_none;
