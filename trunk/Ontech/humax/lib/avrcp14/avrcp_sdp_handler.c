...
static void avrcpHandleProfileResult(AVRCP                  *avrcp, 
                                     ...
                                     bool                   tell_app)
{
    ...
    {        
        if (SdpParseGetProfileVersion(cfm->size_attributes,
                                 (uint8*)cfm->attributes, 0x110e, &sdp_data))
        {    
            status = avrcp_success;
            /* See if this is the metadata extension version of the profile. */
            if (sdp_data == 0x0103 || sdp_data == 0x104)
...
static void avrcpHandleFeaturesResult(AVRCP                 *avrcp,
                const CL_SDP_SERVICE_SEARCH_ATTRIBUTE_CFM_T *cfm,
                                     bool                    tell_app)
{
    uint16 sdp_data = 0;
    ...
    if (cfm->status == sdp_response_success)
    {        
        if (SdpParseGetSupportedFeatures(cfm->size_attributes,
                                         (uint8*)cfm->attributes, &sdp_data))
        {    
            status = avrcp_success;
        }
    }
...
void avrcpRegisterServiceRecord(AVRCP *avrcp)
{
    ...
    if (avrcp->device_type == avrcp_controller)
    {
    ...
    else
    {
        ...
        /* Set correct profile version depending on if the 
           Meta data extensions are enabled. */
        if (avrcp->local_extensions & AVRCP_EXTENSION_METADATA)
        {
           /* Browsing is not supported in Target. If the device is Category 1
              or 3. Set it to v1.3. 1.4 Target is currently supported
             for category   2 or 4 only */
        ...
        /* Insert supported Meta data features */
        if (!SdpParseInsertProfileVersion(size_record, 
                                         service_record_copy, 0x110e, profile))
        {
            /* If we fail to insert the profile version return 
               an error to the app */        
            avrcpSendInitCfmToClient(avrcp->clientTask, avrcp, avrcp_fail);
    ...
    {
        /* Insert supported Meta data features */
        if (!SdpParseInsertSupportedFeatures(size_record,
                                             service_record_copy,
                                             features))
        {
            /* If we fail to insert the supported features return an
                 error to the app */        
            avrcpSendInitCfmToClient(avrcp->clientTask, avrcp, avrcp_fail);
    ...
    ConnectionRegisterServiceRecord(&avrcp->task,
                                     size_record, 
                                    service_record_copy);
}
...
void avrcpHandleSdpRegisterCfm(AVRCP *avrcp, const CL_SDP_REGISTER_CFM_T *cfm)
{
    ...
    else
    {
        ...
        avrcpSendInitCfmToClient(avrcp->clientTask, avrcp, 
...
void avrcpGetProfileVersion(AVRCP *avrcp, bool app_request)
{
    ...
    if (SinkGetBdAddr(avrcp->sink, &my_addr))
    {
        ...
        ConnectionSdpServiceSearchAttributeRequest(&avrcp->task,
                             &my_addr, 0x32, sizeof(AvrcpServiceRequest),
                             (uint8 *) AvrcpServiceRequest,
                             sizeof(profileDescriptorRequest),
                              profileDescriptorRequest);
    }
}
...
void avrcpGetSupportedFeatures(AVRCP *avrcp, bool app_request)
{
    ...
    if (SinkGetBdAddr(avrcp->sink, &my_addr))
    {
        ...
        ConnectionSdpServiceSearchAttributeRequest(&avrcp->task, &my_addr, 
                                  0x32, sizeof(AvrcpServiceRequest),
                                 (uint8 *) AvrcpServiceRequest,
                                 sizeof(supportedFeaturesAttributeRequest), 
                                supportedFeaturesAttributeRequest);
    }
}
