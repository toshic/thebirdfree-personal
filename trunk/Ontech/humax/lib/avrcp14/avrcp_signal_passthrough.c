               ...
               const AVRCP_INTERNAL_PASSTHROUGH_REQ_T   *req)
{
    ...
    if(AVRCP_AVC_MAX_PKT_SIZE > packet_size) 
    {
        ...
        if((packet_size < max_mtu) || (req->opid == opid_vendor_unique))
        {    
                                   ...
                                   uint16 packet_size)
{
   ...
   uint8 transaction;
    ...
    if(packet_size > AVRCP_PASSTHROUGH_HEADER_SIZE)
    {
        avrcp->srcUsed -= (packet_size - AVRCP_PASSTHROUGH_HEADER_SIZE);
    }
    
   op_data_len = ptr[AVRCP_DATA_LEN_OFFSET] ;

   if(op_data_len + AVRCP_PASSTHROUGH_HEADER_SIZE > packet_size)
   {
                                    ...
                                    uint16 packet_size)
{
       ...
       (avrcp->pending == avrcp_previous_group))
    {
       ...
       {
            avrcp->srcUsed -= (packet_size - AVRCP_GROUP_HEADER_SIZE);
       }
    ...
    {
        AVRCP_INFO(("avrcpHandlePassthroughResponse: Wrong pending state: %d.\n",
                    avrcp->pending));
    }
