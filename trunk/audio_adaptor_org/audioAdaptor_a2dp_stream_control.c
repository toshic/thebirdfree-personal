...
{    
    ...
    /* Connect the USB ports with DSP ports and start DSP*/
    if (the_app->a2dp_source == SourceUsb)
    {
        /* Connect the USB port with DSP */
        (void) StreamConnect(PanicNull(StreamUsbEndPointSource(end_point_iso_in)), StreamKalimbaSink(0));
        (void) StreamConnect(StreamKalimbaSource(1), PanicNull(StreamUsbEndPointSink(end_point_iso_out)));
...
void a2dpStreamSetDspIdle(void)
{
    ...
    /* For analogue input, we poweroff DSP instead of setting IDLE mode */
    if(the_app->a2dp_source == SourceUsb)
    ...
    else
    {
        ...
        */
        ...
        AudioConnect(the_app->a2dp_audio_plugin , 
                 ...
                 (the_app->a2dp_channel_mode == a2dp_mono) ? (FALSE) : (TRUE), 
...
void a2dpStreamSetAudioStreamingState (mvdStreamingState streaming_state)
{
    ...
    /* We need set the audio streaming timer only for USB source */
    if ((streaming_state == StreamingPending) && (the_app->a2dp_source == SourceUsb))
    {    /* Submit new timer message */
        MessageSendLater(&the_app->task, APP_AUDIO_STREAMING_TIMER, NULL, (uint32)the_app->audio_streaming_timeout * 1000L);
    }
}
...
void a2dpStreamRestartAudioStream(void)
{
    ...
    {
        if ((the_app->audio_streaming_state != StreamingActive) && the_app->audio_streaming_timeout)                        
        {    
