...
#include <connection.h>
    ...
    static const char *s_app_states[] =
    {
...
*/
static void startDfuModeRequest (void)
{
    setAppState(AppStateEnteringDfu);
    MessageSendLater(&the_app->task, APP_ENTER_DFU_MODE, NULL, 10000);
}
...
{
    if(!(the_app->audioAdaptorPoweredOn) && the_app->a2dp_source == SourceAnalog )
    {
...
/****************************************************************************
NAME
    handleAppPowerOffReq

DESCRIPTION
    Handles the application power off request message.
    
*/
static void handleAppPowerOffReq(void)
{
    if (the_app->PowerOffIsEnabled && the_app->a2dp_source == SourceAnalog )
    {
        DEBUG_EVENT(("Powering off...\n"));
#ifndef NO_CHARGER_TRAPS                
        PioSetPsuRegulator ( TRUE ) ;
#else
        PioSet(POWER_HOLD,0);
        ...
        the_app->audioAdaptorPoweredOn = FALSE;
        
        /* Make sure all connection activity is stopped */
        profileSlcCheckPoweredOff(NULL);
...
/****************************************************************************
NAME
    handleAppLimboTimeout

DESCRIPTION
    Handles the application limbo timeout message.
    
*/
static void handleAppLimboTimeout(void)
{
...
{        
    /* Reset the stored Paired Device List in PS Key and Buffer*/
...
static void handleAppDeviceConnectReq(bool disconnect_current)
{
    switch (the_app->app_state)
    {
        ...
        case AppStateInCall:
        {          
            if (configureGetBdaddrNextDisconnectedDevice(&the_app->connect_bdaddr))
            {
...
/****************************************************************************
NAME
    handleAppDeviceConnectSecondReq

DESCRIPTION
    Handles the application connect second device message.
    
*/
static void handleAppDeviceConnectSecondReq(void)
{        
#ifdef DUAL_STREAM            
    uint16 i;
    uint16 active_connections = 0;
    bdaddr addr;
            
    /* if no devices connected - connect as previously
       if 1 device connected - if another devices in paired list then connect to them
                             - if no other devices then discover new device and connect it
       if 2 devices connected - do nothing */
    active_connections = 0;
    for (i = 0; i < MAX_NUM_DEV_CONNECTIONS; i++)
    {
        if (the_app->dev_inst[i] != NULL)
        {  
            if (!profileSlcAreAllProfilesDisconnected(the_app->dev_inst[i]))
                active_connections++;                
        }
    }
    switch (active_connections)
    {
        ...
        {
            /* if another device */
            if (configureGetBdaddrNextDisconnectedDevice(&addr))
            {
                MAKE_APP_MESSAGE(APP_DEVICE_CONNECT_REQ);
                message->disconnect_current = FALSE;
                MessageSend(&the_app->task, APP_DEVICE_CONNECT_REQ, message);
            }
            else
            {
                /* no other device, so discover new device without disconnecting the first */
...
{            
    ...
    usbHidIssueHidCommand(AppEventMicActive);
...
{                    
    if (a2dpSlcIsMediaOpen() && the_app->audio_streaming_state!=StreamingActive && the_app->audio_streaming_timeout)
    ...
    switch ( the_app->app_state )
    {
        ...
        case AppStateInCall:     /* This state reached if an active call is in progress or an incoming/outgoing call is being setup */
        {
            ...
            {
                the_app->voip_call_active = FALSE;
                usbHidIssueHidCommand(AppEventMicInactive);
...
{            
    usbHidIssueHidCommand(AppEventSpeakerActive);
...
{            
    usbHidIssueHidCommand(AppEventSpeakerInactive);
    
    if (a2dpSlcIsMediaOpen() && the_app->audio_streaming_timeout)
...
{    
    switch(id)
    {
        ...
        case APP_CHARGER_CONNECTED:
        {
            DEBUG_EVENT(("APP_CHARGER_CONNECTED\n"));
            powerManagerChargerConnected();
            break;
        }
        case APP_CHARGER_DISCONNECTED:
        {
            DEBUG_EVENT(("APP_CHARGER_CONNECTED\n"));
        ...
        {
            /*DEBUG_EVENT(("APP_CHARGER_MONITOR\n"));*/
            chargerHandler();
            break;
        }
        case APP_CHECK_FOR_LOW_BATT:
        {
            DEBUG_EVENT(("APP_CHECK_FOR_LOW_BATT\n"));
            batteryCheckLowBatt(); 
            break;
        }
        case APP_LOW_BATTERY:
        {
            DEBUG_EVENT(("APP_LOW_BATTERY\n"));
            /* Just change the LED flash pattern */
            if(the_app->charger_state == disconnected)
            {
                setAppState (AppStateLowBattery);
            }
            break;
        }    
        case APP_FAST_CHARGE:
        {
            /*DEBUG_EVENT(("APP_FAST_CHARGE\n"));*/
            break;
        }
        case APP_TRICKLE_CHARGE:
        {
            /*DEBUG_EVENT(("APP_TRICKLE_CHARGE\n"));*/
            break;
        }
        case APP_OK_BATTERY:
        {
            DEBUG_EVENT(("APP_OK_BATTERY\n"));
            break;
        }
        ...
        case APP_DEVICE_CONNECT_SECOND_REQ:    
        {
            DEBUG_EVENT(("APP_DEVICE_CONNECT_SECOND_REQ\n"));
            handleAppDeviceConnectSecondReq();
            break;
        }
