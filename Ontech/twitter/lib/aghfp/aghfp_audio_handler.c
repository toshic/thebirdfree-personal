...
{    
    ...
    if ( !response )
    {
        ...
        if(SinkGetBdAddr(aghfp->rfcomm_sink, &bd_addr))
        {
            ConnectionSyncConnectResponse(&aghfp->task, &bd_addr, FALSE, 0);
        }
        /* 
    ...
    else
    {
        ...
        */
        if(SinkGetBdAddr(aghfp->rfcomm_sink, &bd_addr))
            ConnectionSyncConnectResponse(&aghfp->task, &bd_addr, TRUE, &config_params);
    }
}
         ...
         aghfp->audio_connection_state==aghfp_audio_accepting )
    {
    	...
    	else 
    	{
        	...
        	if ( cfm->status<=hci_error_host_timeout || 
        	     ...
        	     aghfp->audio_connection_state==aghfp_audio_accepting )
        	{
                if (aghfp->audio_connection_state != aghfp_audio_accepting)
                    aghfp->audio_connection_state = aghfp_audio_disconnected;
