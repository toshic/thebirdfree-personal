...
static void handleInternalMessage ( Task task , MessageId id, Message message ) 	;
	...
	/*the plugin task*/
const TaskData csr_mp3_encoder_plugin = { message_handler };
...
static void handleAudioMessage ( Task task , MessageId id, Message message ) 	
{
	switch (id)
	{
		case (AUDIO_PLUGIN_CONNECT_MSG ):
		{
			...
			{		/*connect the audio*/
				CsrMp3EncoderPluginConnect(   connect_message->audio_sink , 
		...
		case (AUDIO_PLUGIN_SET_MODE_MSG ):
		{
            ...
            {
                CsrMp3EncoderPluginSetMode(mode_message->mode , mode_message->params) ;
            }
		}
		break ;
		...
		case (AUDIO_PLUGIN_SET_VOLUME_MSG ): 
		{
            ...
            {
                CsrMp3EncoderPluginSetVolume ( volume_message->volume ) ;
            }			
		}		
		break ;
		...
		case (AUDIO_PLUGIN_PLAY_TONE_MSG ):
		{
			...
			{
				PRINT(("MP3DSP: Tone start\n"));				
				AUDIO_BUSY = (TaskData*) &csr_mp3_encoder_plugin;					
				CsrMp3EncoderPluginPlayTone ( tone_message->tone, tone_message->codec_task  , 					  
...
static void handleInternalMessage ( Task task , MessageId id, Message message ) 	
{
	PRINT(("MP3DSP: tone complete\n")) ;
		/*the tone has completed*/  
	AUDIO_BUSY = NULL ;
    
    CsrMp3EncoderPluginToneComplete() ; 	
}
