/****************************************************************************
Copyright (C) Cambridge Silicon Radio Limited 2005-2009
Part of BlueLab 4.1.2-Release

FILE NAME
	hid_control_handler.c        
DESCRIPTION
	handles transmitting and receiving HID packets on the control channel.
*/

#include <source.h>
#include <sink.h>
#include <stream.h>
#include <string.h>

#include "hid.h"
#include "hid_private.h"
#include "hid_common.h"
#include "hid_connect.h"
#include "hid_control_handler.h"

/* Local Function Prototypes */
static void hidHandshakeSend(HID *hid, hid_status result);
static void hidPacketSend(HID *hid, uint8 header, const uint8 *data, int length);
static hidPacketStatus hidPacketStart(HID *hid, uint16 packet, const uint8 *data, uint8 length);
static void hidPacketStop(HID *hid, const uint8 *data, int length);
static hidPacketStatus hidPacketAppend(HID *hid, const uint8 *data, int length);
static void hidPacketFailed(HID *hid, uint16 packet, hid_status status);

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleHidGetReportInd
DESCRIPTION
	This function handles the HID_GET_REPORT packet.
RETURNS
	void
*/
static void hidHandleHidGetReportInd(HID *hid, const uint8 *data, int length)
{
    MAKE_HID_MESSAGE_OR_NULL(HID_GET_REPORT_IND);
    HID_PRINT(("hidHandleHidGetReportInd, length %d\n", length));
    
	/* Check we allocated message */
	if (message != NULL)
	{
		/* Get report type */
		message->report_type = data[0] & 0x03;
    
        /* Trim the GET_REPORT packet to its maximum permissable length */
        if(length > HID_GET_REPORT_HEADER_MAX_LEN)
            length = HID_GET_REPORT_HEADER_MAX_LEN;
        
		/* Check if we have a report id, we do if length is even */
		if (length & 0x01)
			message->report_id = 0;
		else
			message->report_id = data[1];

		/* Check if buffer size is specified */
		if (data[0] & (1 << 3))
		{
	        /* NOTE: Limit for payload WITHOUT header */
			message->report_length = ((data[length - 1] << 8) | data[length - 2]);
		}
		else
		{
			/* Report the size as 0xFFFF - should be big enough for the response! */
			message->report_length = 0xFFFF;
		}
    
		/* Send HID_GET_REPORT_IND to application */
		message->hid = hid;
		MessageSend(hid->app_task, HID_GET_REPORT_IND, message);
	}
	else
	{
		hidPacketFailed(hid, HID_GET_REPORT_IND, hid_failed);
	}
}
#endif

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleHidSetReportInd
DESCRIPTION
	This function handles the HID_SET_REPORT packet.
RETURNS
	void
*/
static void hidHandleHidSetReportInd(HID *hid, const uint8 *data, int length)
{
	MAKE_HID_MESSAGE_WITH_LEN_OR_NULL(HID_SET_REPORT_IND, length - 1);        
    HID_PRINT(("hidHandleHidSetReportInd, length %d\n", length));
    
	/* Check if we allocated message */
    if (message != NULL)
	{
		/* Get report type */
	    message->report_type = data[0] & 0x03;
    
		/* Get report length */
		message->size_report_data = length - 1;
    
		/* Copy report */
		memcpy(message->report_data, &data[1], length - 1);
    
		/* Send HID_SET_REPORT_IND to application */
		message->hid = hid;
		MessageSend(hid->app_task, HID_SET_REPORT_IND, message);
	}
	else
	{
		hidPacketFailed(hid, HID_SET_REPORT_IND, hid_failed);
	}
}
#endif

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleHidSetReportInd
DESCRIPTION
	This function handles the HID_SET_REPORT packet.
RETURNS
	void
*/
static void hidHandleHidDataInd(HID *hid, const uint8 *data, int length)
{
	MAKE_HID_MESSAGE_WITH_LEN_OR_NULL(HID_DATA_IND, length - 1);        
    HID_PRINT(("hidHandleHidDataInd, length %d\n", length));
    
	/* Check if we allocated message */
    if (message != NULL)
	{
		/* Get report type */
	    message->report_type = data[0] & 0x03;
    
		/* Get report length */
		message->size_report_data = length - 1;
    
		/* Copy report */
		memcpy(message->report_data, &data[1], length - 1);
    
		/* Send HID_DATA_IND to application */
		message->hid = hid;
		MessageSend(hid->app_task, HID_DATA_IND, message);
	}
}
#endif

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleHidGetIdleInd
DESCRIPTION
	This function handles the HID_GET_IDLE packet.
RETURNS
	void
*/
static void hidHandleHidGetIdleInd(HID *hid, const uint8 *data, int length)
{
    MAKE_HID_MESSAGE_OR_NULL(HID_GET_IDLE_IND);
    HID_PRINT(("hidHandleHidGetIdleInd\n"));
        
	/* Check if we allocated message */
	if (message != NULL)
	{
		/* Send HID_GET_IDLE_IND to application */
		message->hid = hid;
		MessageSend(hid->app_task, HID_GET_IDLE_IND, message);
	}
	else
	{
		hidPacketFailed(hid, HID_GET_IDLE_IND, hid_failed);
	}
}
#endif

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleHidSetIdleInd
DESCRIPTION
	This function handles the HID_SET_IDLE packet.
RETURNS
	void
*/
static void hidHandleHidSetIdleInd(HID *hid, const uint8 *data, int length)
{
    MAKE_HID_MESSAGE_OR_NULL(HID_SET_IDLE_IND);
    HID_PRINT(("hidHandleHidSetIdleInd\n"));
        
	/* Check if we allocated message */
	if (message != NULL)
	{
		/* Send HID_SET_IDLE_IND to application */
		message->hid = hid;
		message->idle_rate = data[1];
		MessageSend(hid->app_task, HID_SET_IDLE_IND, message);
	}
	else
	{
		hidPacketFailed(hid, HID_SET_IDLE_IND, hid_failed);
	}
}
#endif

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleHidGetProtocolInd
DESCRIPTION
	This function handles the HID_GET_PROTOCOL packet.
RETURNS
	void
*/
static void hidHandleHidGetProtocolInd(HID *hid, const uint8 *data, int length)
{
    MAKE_HID_MESSAGE_OR_NULL(HID_GET_PROTOCOL_IND);
    HID_PRINT(("hidHandleHidGetProtocolInd\n"));
        
	/* Check if we allocate message */
	if (message != NULL)
	{
		/* Send HID_GET_PROTOCOL_IND to application */
		message->hid = hid;
		MessageSend(hid->app_task, HID_GET_PROTOCOL_IND, message);
	}
	else
	{
		hidPacketFailed(hid, HID_GET_PROTOCOL_IND, hid_failed);
	}
}
#endif

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleHidSetProtocolInd
DESCRIPTION
	This function handles the HID_SET_PROTOCOL packet.
RETURNS
	void
*/
static void hidHandleHidSetProtocolInd(HID *hid, const uint8 *data, int length)
{
    MAKE_HID_MESSAGE_OR_NULL(HID_SET_PROTOCOL_IND);
    HID_PRINT(("hidHandleHidSetProtocolInd\n"));
        
	/* Check if we allocated message */
	if (message != NULL)
	{
		/* Send HID_SET_PROTOCOL_IND to application */
		message->hid = hid;
		message->protocol = data[0] & 0x01;
		MessageSend(hid->app_task, HID_SET_PROTOCOL_IND, message);
	}
	else
	{
		hidPacketFailed(hid, HID_SET_PROTOCOL_IND, hid_failed);
	}
}
#endif

#ifdef HID_HOST
/****************************************************************************
NAME	
	hidHandleHidGetProtocolCfm
DESCRIPTION
	This function handles the confirmation of the HID_GET_PROTOCOL packet.
RETURNS
	void
*/
static void hidHandleHidGetProtocolCfm(HID *hid, const uint8 *data, int length)
{
    MAKE_HID_MESSAGE(HID_GET_PROTOCOL_CFM);
    HID_PRINT(("hidHandleHidGetProtocolCfm, length=%d\n", length));
      
    /* Send HID_GET_PROTOCOL_CFM to application */
    message->hid = hid;
    message->status = (length > 0) ? hid_success : hid_failed;
    message->protocol = (length > 0) ? (data[0] & 0x01) : 0;
    MessageSend(hid->app_task, HID_GET_PROTOCOL_CFM, message);
}
#endif

#ifdef HID_HOST
/****************************************************************************
NAME	
	hidHandleHidGetIdleCfm
DESCRIPTION
	This function handles the confirmation of the HID_GET_IDLE packet.
RETURNS
	void
*/
static void hidHandleHidGetIdleCfm(HID *hid, const uint8 *data, int length)
{
	MAKE_HID_MESSAGE(HID_GET_IDLE_CFM);
    HID_PRINT(("hidHandleHidGetIdleCfm, length=%d\n", length));
       
    /* Send HID_GET_IDLE_CFM to application */
    message->hid = hid;
    message->status = (length > 0) ? hid_success : hid_failed;
    message->idle_rate = (length > 0) ? data[0] : 0;
    MessageSend(hid->app_task, HID_GET_IDLE_CFM, message);
}
#endif

#ifdef HID_HOST
/****************************************************************************
NAME	
	hidHandleHidGetReportCfm
DESCRIPTION
	This function handles the confirmation of the HID_GET_REPORT packet.
RETURNS
	void
*/
static void hidHandleHidGetReportCfm(HID *hid, const uint8 *data, int length)
{
	MAKE_HID_MESSAGE_WITH_LEN(HID_GET_REPORT_CFM, length);
    HID_PRINT(("hidHandleHidGetReportCfm\n"));
                
    /* Send HID_GET_REPORT_CFM to application */
    message->hid = hid;
                
    /* Copy data into packet */
    memcpy(message->report_data, data, length);
    message->size_report_data = length;
                    
    /* Indicate success */
    message->status = hid_success;               
                
    /* Send message to application */
    MessageSend(hid->app_task, HID_GET_REPORT_CFM, message);
}
#endif

/****************************************************************************
NAME	
	hidPacketFailed
DESCRIPTION
	This function is called to send a confirmation to a request that has
    failed.
RETURNS
	void
*/
static void hidPacketFailed(HID *hid, uint16 packet, hid_status status)
{
	HID_PRINT(("hidPacketFailed\n"));

	/* Check if any packet pending */
	if (hid->packet_data != NULL)
	{
		/* Free packet first, so we have memory to send error message */
		free(hid->packet_data);
		hid->packet_data = NULL;
	}

	/* Handle packet type */
    switch (packet)
    {
#ifdef HID_HOST
		case HID_GET_REPORT_CFM:
        {
            MAKE_HID_MESSAGE(HID_GET_REPORT_CFM);
        
            /* Send HID_GET_REPORT_CFM application */
            message->hid = hid;
            message->status = status;
            MessageSend(hid->app_task, HID_GET_REPORT_CFM, message);
        }
        break;

        case HID_SET_REPORT_CFM:
        {
            MAKE_HID_MESSAGE(HID_SET_REPORT_CFM);
        
            /* Send HID_SET_REPORT_CFM application */
            message->hid = hid;
            message->status = status;
            MessageSend(hid->app_task, HID_SET_REPORT_CFM, message);
        }
        break;
        
        case HID_GET_PROTOCOL_CFM:
        {
            MAKE_HID_MESSAGE(HID_GET_PROTOCOL_CFM);
        
            /* Send HID_GET_PROTOCOL_CFM application */
            message->hid = hid;
            message->status = status;
            MessageSend(hid->app_task, HID_GET_PROTOCOL_CFM, message);
        }
        break;
        
        case HID_SET_PROTOCOL_CFM:
        {
            MAKE_HID_MESSAGE(HID_SET_PROTOCOL_CFM);
        
            /* Send HID_SET_PROTOCOL_CFM application */
            message->hid = hid;
            message->status = status;
            MessageSend(hid->app_task, HID_SET_PROTOCOL_CFM, message);
        }
        break;
        
        case HID_GET_IDLE_CFM:
        {
            MAKE_HID_MESSAGE(HID_GET_IDLE_CFM);
        
            /* Send HID_GET_IDLE_CFM application */
            message->hid = hid;
            message->status = status;
            MessageSend(hid->app_task, HID_GET_IDLE_CFM, message);
        }
        break;
        
        case HID_SET_IDLE_CFM:
        {
            MAKE_HID_MESSAGE(HID_SET_IDLE_CFM);
        
            /* Send HID_SET_IDLE_CFM application */
            message->hid = hid;
            message->status = status;
            MessageSend(hid->app_task, HID_SET_IDLE_CFM, message);
        }
        break;
#endif

#ifdef HID_DEVICE
		case HID_GET_REPORT_IND:
		case HID_SET_REPORT_IND:
		case HID_GET_PROTOCOL_IND:
		case HID_SET_PROTOCOL_IND:
		case HID_SET_IDLE_IND:
		case HID_GET_IDLE_IND:
		{
			/* Tell host */
            hidHandshakeSend(hid, status);
		}
		break;
#endif
        default:
			HID_DEBUG(("hidPacketFailed, unknown packet=%d\n", packet));
			break;
    }

    /* Cancel pending HID request timer */
    MessageCancelAll(&hid->task, HID_INTERNAL_REQUEST_TIMEOUT_IND);

	/* Clear packet pending */
    hid->packet_pending = 0;    
}

/****************************************************************************
NAME	
	hidPacketStop
DESCRIPTION
	This function is called when a resposne to a HID request has been
	received. 
RETURNS
	void
*/
void static hidPacketStop(HID *hid, const uint8 *data, int length)
{
	HID_PRINT(("hidPacketStop\n"));

	/* Handle packet type */
	switch (hid->packet_pending)
	{
#ifdef HID_DEVICE
		case HID_GET_REPORT_IND:
			hidHandleHidGetReportInd(hid, data, length);
    	    break;
		case HID_SET_REPORT_IND:
			hidHandleHidSetReportInd(hid, data, length);
			break;
		case HID_GET_PROTOCOL_IND:
			hidHandleHidGetProtocolInd(hid, data, length);
        	break;
		case HID_SET_PROTOCOL_IND:
			hidHandleHidSetProtocolInd(hid, data, length);
            break;                
		case HID_GET_IDLE_IND:
			hidHandleHidGetIdleInd(hid, data, length);
        	break;
		case HID_SET_IDLE_IND:
    	    hidHandleHidSetIdleInd(hid, data, length);
        	break;
#endif
#ifdef HID_HOST
		case HID_GET_REPORT_CFM:
			hidHandleHidGetReportCfm(hid, data, length);
			hid->packet_pending = 0;    
    	    break;
		case HID_GET_PROTOCOL_CFM:
			hidHandleHidGetProtocolCfm(hid, data, length);
			hid->packet_pending = 0;    
        	break;
		case HID_GET_IDLE_CFM:
			hidHandleHidGetIdleCfm(hid, data, length);
			hid->packet_pending = 0;    
        	break;
		case HID_SET_PROTOCOL_CFM:
		case HID_SET_REPORT_CFM:
		case HID_SET_IDLE_CFM:
			hid->packet_pending = 0;    
			hidPacketFailed(hid, hid->packet_pending, hid_failed);
        	return;
#endif
	}
	
	/* Free any packet */
	if (hid->packet_data != NULL)
	{
		free(hid->packet_data);
		hid->packet_data = NULL;
	}

    /* Cancel pending HID request timer */
    MessageCancelAll(&hid->task, HID_INTERNAL_REQUEST_TIMEOUT_IND);
}

/****************************************************************************
NAME	
	hidPacketStart
DESCRIPTION
	This function attempts to start a control request, if a previous request
    is still pwending then this function will return indicating a failure.
    otherwise it will store the pending request type and start a timer.    
RETURNS
	TRUE if request can be started, or FALSE if previous request still pending.
*/
static hidPacketStatus hidPacketStart(HID *hid, uint16 packet, const uint8 *data, uint8 length)
{
	/* Check if already request in progress */
	if (hid->packet_pending != 0)
    {
		HID_PRINT(("hidPacketStart packet=%d (BUSY)\n", packet));

		/* Return indicating packet was rejected because we are busy */
        return hidPacketStatusBusy;
    }    

	HID_PRINT(("hidPacketStart packet=%d\n", packet));

    /* Store packet type */
    hid->packet_pending = packet;
	hid->packet_data = NULL;
    hid->packet_size = 0;

	/* Check if we need to allocate memory for packet data */
	if (data != NULL)
	{
		/* Check if we have complete packet */
		if (length < HID_L2CAP_MTU)
		{
			/* Packet completed */
			hidPacketStop(hid, data, length);

			/* Return indicating we have completed the packet */
			return hidPacketStatusCompleted;
		}
		else
		{
			/* Allocate memory for packet */
			hid->packet_data = malloc(length);
                
			/* Check if allocation was successful */
			if (hid->packet_data != NULL)
			{
				/* Store first part of packet */
				memcpy(hid->packet_data, data, length);
				hid->packet_size = length;

			    /* Start timer */
				HID_PRINT(("hidPacketStart, start timer\n"));
				MessageSendLater(&hid->task, HID_INTERNAL_REQUEST_TIMEOUT_IND, 0, D_SEC(HID_REQUEST_TIMEOUT));

				/* Return indicating packet is pending */
				return hidPacketStatusPending;
			}
			else
			{
				/* Packet has failed due to insufficent memory */
			    hidPacketFailed(hid, packet, hid_failed);

				/* Return indicating packet has failed */
				return hidPacketStatusFailed;
			}
		}
	}
	else
	{
	    /* Start timer */
		HID_PRINT(("hidPacketStart, start timer\n"));
		MessageSendLater(&hid->task, HID_INTERNAL_REQUEST_TIMEOUT_IND, 0, D_SEC(HID_REQUEST_TIMEOUT));

		/* Return indicating packet is pending */
		return hidPacketStatusPending;
	}
}

/****************************************************************************
NAME	
	hidPacketAppend
DESCRIPTION
	This function appends received data to the end of the current packet.
RETURNS
	void
*/
static hidPacketStatus hidPacketAppend(HID *hid, const uint8 *data, int length)
{
	/* Check if we should be handling a packet */
	if (hid->packet_pending)
	{
		/* Attempt to allocate a bigger block */
		uint8 *packet_data = realloc(hid->packet_data, hid->packet_size + length);
		if (packet_data != NULL)
		{
			/* Store pointer to new block */
			hid->packet_data = packet_data;
                    
			/* Copy data to end of block */
			memcpy(packet_data + hid->packet_size, data, length);
			hid->packet_size += length;
                    
			/* Check if this is the last packet */
			if (length < HID_L2CAP_MTU)
			{
				/* Packet completed */
				hidPacketStop(hid, hid->packet_data, hid->packet_size);

				/* Return indicating packet completed */
				return hidPacketStatusCompleted;
			}
			else
			{
				/* Return indicating packet still pending */
				return hidPacketStatusPending;
			}
		}
		else
		{
			/* No memory, report failure */
			hidPacketFailed(hid, hid->packet_pending, hid_failed);

			/* Return indicating failure */
			return hidPacketStatusFailed;
		}
	}
	else
	{
		return hidPacketStatusFailed;
	}
}

/****************************************************************************
NAME	
	hidPacketSend
DESCRIPTION
	This function sends data packets over the control channel and carries out segmentation
	if necessary.
RETURNS
	void
*/
static void hidPacketSend(HID *hid, uint8 header, const uint8 *data, int length)
{
	Sink sink = hid->connection[HID_CON_CONTROL].con.sink;
    HID_PRINT(("hidPacketSend, header=%x, length=%d\n", header, length));

	/* Check if we have a sink */
	if (sink)
    {
        int mtu = hid->connection[HID_CON_CONTROL].remote_mtu;   

		/* Build packets */
		for (;;)
		{
		    /* Get pointer to stream */
			uint8 *ptr = SinkMap(sink);

			/* Get packet data size (excludes header) */
			int size = ((length + 1) > mtu) ? (mtu - 1) : length;

			/* Claim space in sink */
			uint16 offset = SinkClaim(sink, size + 1);
			if (offset == 0xFFFF)
				break;

			/* Store header */
			ptr[offset] = header;

			/* Copy data into packet */
			memcpy(&ptr[offset + 1], data, size);

			/* Flush packet */
			SinkFlush(sink, size + 1);

			/* Quit loop if we just handled a packet smaller than the MTU */
			if (size < (mtu - 1))
				break;

			/* Update header */
			header = (HID_DATC << 4) | (header & 0x0F);

			/* Adjust length remaining */
			length -= size;
			data += size;
		}
    }
}

/****************************************************************************
NAME	
	hidControlGetMinimumPacketSize
DESCRIPTION
	This function returns minimum packet length (including header for a packet)    
RETURNS
	void
*/
static int hidControlGetMinimumPacketSize(uint8 header)
{
	switch (header)
	{
		case HID_CONTROL:
			return 1;
		case HID_GET_REPORT:
			return 1;
		case HID_SET_REPORT:
			return 2;
		case HID_GET_PROTOCOL:
			return 1;
		case HID_SET_PROTOCOL:
			return 1;
		case HID_GET_IDLE:
			return 1;
		case HID_SET_IDLE:
			return 2;
#ifdef HID_HOST
        case HID_HANDSHAKE:
            return 1;
        case HID_DATA:
            return 2;
        case HID_DATC:
            return 2;
#endif
		default:
			return 0;
	}
}

/****************************************************************************
NAME	
	hidHandleHidControl
DESCRIPTION
	This function handles the HID_CONTROL packet.
RETURNS
	void
*/
static void hidHandleHidControl(HID *hid, hid_control_op operation)
{
    HID_PRINT(("hidHandleHidControl, operation=%d\n", operation));
    switch (operation)
    {
        case hid_control_op_nop:
            break;
#ifdef HID_DEVICE            
        case hid_control_op_hard_reset:
        case hid_control_op_soft_reset:
        case hid_control_op_suspend:
        case hid_control_op_exit_suspend:
        {
           	MAKE_HID_MESSAGE(HID_CONTROL_IND);
        
            /* Send HID_CONTROL_IND to application */
       	    message->hid = hid;
            message->operation = operation;
           	MessageSend(hid->app_task, HID_CONTROL_IND, message);
        }
        break;
#endif            
        case hid_control_op_unplug:
        {
			/* Set disconnect reason */
			hid->disconnect_status = hid_disconnect_virtual_unplug;

			/* Move to disconnecting local state */
			hidSetState(hid, hidDisconnectingLocal);
        }
        break;
            
        default:
        {
            /* Invalid operation */
            hidHandshakeSend(hid, hid_invalid_param);
        }
        break;
    }
}

#ifdef HID_HOST
/****************************************************************************
NAME	
	hidHandleHidHandshake
DESCRIPTION
	This function handles the HID_HANDSHAKE packet. 
RETURNS
	void
*/
static void hidHandleHidHandshake(HID *hid, hid_status status)
{
    HID_PRINT(("hidHandleHidHandshake, pending packet=%x, status=%d\n", hid->packet_pending, status));
    
    /* Handle response to pending request */
    switch (hid->packet_pending)
    {
        /* We should never get a successful handshake for these requests,
           only a failure or a DATA/DATC response */
        case HID_GET_PROTOCOL_CFM:
        case HID_GET_IDLE_CFM:
        case HID_GET_REPORT_CFM:
        {
            /* Send confirmation to application, if we get a handshake
               indicating success then the device does follow the spec */
            hidPacketFailed(hid, hid->packet_pending, (status == hid_success) ? hid_failed : status);
        }
        break;

        /* Handshake is always the required response for these requests */
        case HID_SET_REPORT_CFM:
        case HID_SET_PROTOCOL_CFM:
        case HID_SET_IDLE_CFM:
        {
            /* Send confirmation to request */
            hidPacketFailed(hid, hid->packet_pending, status);
        }
		break;

        default:
		{
			/* Not expecting handshake, so just ignore it */
			HID_PRINT(("Received unexpected handshake\n"));
		}
		break;
    }
}
#endif

/****************************************************************************
NAME	
	hidHandleControlPacket
DESCRIPTION
	This function decodes packets received in the control channel.
RETURNS
	void
*/
void hidHandleControlPacket(HID *hid, Source source)
{
    int packet_size;
    Sink sink = StreamSinkFromSource(source);

    HID_PRINT(("hidHandleControlPacket, sink=%x\n", (int)sink));

    /* Check source is for control channel */
	if (sink == hid->connection[HID_CON_CONTROL].con.sink)
    {
	    /* Loop, pulling packets from source */
    	while ((packet_size = SourceBoundary(source))) 
    	{
			const uint8 *ptr = SourceMap(source);
			uint8 header = ptr[0] >> 4;
        
        	/* Check packet is at least minimum size */
	        if (packet_size >= hidControlGetMinimumPacketSize(header))
    	    {
        	    /* Decode header */
            	switch (header)
	            {
					case HID_CONTROL:
					{
						/* Handle control packet */
        	            hidHandleHidControl(hid, ptr[0] & 0x0F);
					}
            	    break;
#ifdef HID_DEVICE          
                	case HID_GET_REPORT:
        	        case HID_SET_REPORT:
	                case HID_GET_PROTOCOL:
            	    case HID_SET_PROTOCOL:
	                case HID_GET_IDLE:
	                case HID_SET_IDLE:
					{
						hidPacketStatus packet_status;

						/* Attempt to start packet */
						packet_status = hidPacketStart(hid, header + HID_IND_BASE, ptr, packet_size);

						/* Check if device is currently busy */
						if (packet_status == hidPacketStatusBusy)
							hidHandshakeSend(hid, hid_busy);
					}
    	            break;                
#endif /* HID_DEVICE */
					case HID_DATC:
					{
						hidPacketStatus packet_status;

						/* Append to end of packet */
    	                packet_status = hidPacketAppend(hid, &ptr[1], packet_size - 1);
					}
					break;
#ifdef HID_HOST
	                case HID_DATA:
					{
						hidPacketStatus packet_status;

						/* Append data to packet */
						packet_status = hidPacketAppend(hid, &ptr[1], packet_size - 1);
					}
        	        break;                                        

	                case HID_HANDSHAKE:
					{
						/* Handle handshake */
    	                hidHandleHidHandshake(hid, ptr[0] & 0x0F);
					}
        	        break;
#endif /* HID_HOST */
	                default:
					{
    	                /* Unknown control packet, return unsupported */
        	            hidHandshakeSend(hid, hid_unsupported);
					}
					break;
	            }
    	    }
        	else
	        {
    	        /* Invalid packet size */
        	    hidHandshakeSend(hid, hid_invalid_param);
	        }
        
  			/* Discard the packet */
			SourceDrop(source, packet_size);
    	}
    }
    /* Check source is for interrupt channel */
    else if (sink == hid->connection[HID_CON_INTERRUPT].con.sink)
    {
#ifdef HID_DEVICE
		/* Get the size available in the source now!! */
		int16 size = SourceSize(source);
 
		/* Loop, pulling packets from source */ 
		while (size > 0)
		{
			  int16 packet_size = SourceBoundary(source);
			  const uint8 *ptr = SourceMap(source);
			  uint8 header = ptr[0] >> 4;
        
			  /* Check if we got HID_DATA packet */
			  if ((header == HID_DATA) && (packet_size >= hidControlGetMinimumPacketSize(header)))
			  {
			      /* Pass packet to application as HID_DATA_IND */
			      hidHandleHidDataInd(hid, ptr, packet_size);
			  }
 
			  /* Discard the packet */
			  SourceDrop(source, packet_size);
 
			  /* Decrement the size counter */
			  size -= packet_size;
		}
#else
		/* Just leave packet in source */
#endif
    }
    else
    	Panic();
}

/****************************************************************************
NAME	
	hidHandshakeSend
DESCRIPTION
	This function sends a handshake packet to the host.
RETURNS
	void
*/
static void hidHandshakeSend(HID *hid, hid_status result)
{
	HID_PRINT(("hidHandshakeSend, result=%d\n", result));
    hidPacketSend(hid, (HID_HANDSHAKE << 4) | result, 0, 0);
}

/****************************************************************************
NAME	
	hidHandleInternalControlRequest
DESCRIPTION
	This function handles the HID_INTERNAL_CONTROL_REQ message.
RETURNS
	void
*/
void hidHandleInternalControlRequest(HID *hid, HID_INTERNAL_CONTROL_REQ_T *req)
{
#ifdef HID_DEVICE
	/* Device can only send VIRTUAL_UNPLUG */
	PanicFalse(req->op == hid_control_op_unplug);
#endif
	hidPacketSend(hid, (HID_CONTROL << 4) | req->op, 0, 0);
}

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleInternalGetIdleResponse
DESCRIPTION
	This function handles the HID_INTERNAL_GET_IDLE_RES message.
RETURNS
	void
*/
void hidHandleInternalGetIdleResponse(HID *hid, HID_INTERNAL_GET_IDLE_RES_T *res)
{
	hid->packet_pending = 0;    
    if (res->status == hid_success)
		hidPacketSend(hid, (HID_DATA << 4), &res->idle_rate, 1);
    else
        hidHandshakeSend(hid, res->status);
}
#endif

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleInternalSetIdleResponse
DESCRIPTION
	This function handles the HID_INTERNAL_SET_IDLE_RES messages.
RETURNS
	void
*/
void hidHandleInternalSetIdleResponse(HID *hid, HID_INTERNAL_SET_IDLE_RES_T *res)
{
	hid->packet_pending = 0;    
    hidHandshakeSend(hid, res->status);
}
#endif

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleInternalGetProtocolResponse
DESCRIPTION
	This function handles the HID_INTERNAL_GET_PROTOCOL_RES message.
RETURNS
	void
*/
void hidHandleInternalGetProtocolResponse(HID *hid, HID_INTERNAL_GET_PROTOCOL_RES_T *res)
{
	hid->packet_pending = 0;    
    if (res->status == hid_success)
    {
        uint8 protocol = res->protocol;
		hidPacketSend(hid, (HID_DATA << 4), &protocol, 1);
    }
    else
        hidHandshakeSend(hid, res->status);
}
#endif

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleInternalSetProtocolResponse
DESCRIPTION
	This function handles the HID_INTERNAL_SET_PROTOCOL_RES message.
RETURNS
	void
*/
void hidHandleInternalSetProtocolResponse(HID *hid, HID_INTERNAL_SET_PROTOCOL_RES_T *res)
{
	hid->packet_pending = 0;    
    hidHandshakeSend(hid, res->status);
}
#endif

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleInternalGetReportResponse
DESCRIPTION
	This function handles the HID_INTERNAL_GET_REPORT_RES message.
RETURNS
	void
*/
void hidHandleInternalGetReportResponse(HID *hid, HID_INTERNAL_GET_REPORT_RES_T *res)
{
	hid->packet_pending = 0;    
    if (res->status == hid_success)
    {
		/* Send the report on the control channel */
		hidPacketSend(hid, (HID_DATA << 4) | res->report_type, res->report_data, res->report_length);
    }
    else
        hidHandshakeSend(hid, res->status);
}
#endif

#ifdef HID_DEVICE
/****************************************************************************
NAME	
	hidHandleInternalSetReportResponse
DESCRIPTION
	This function handles the HID_INTERNAL_SET_REPORT_RES message.
RETURNS
	void
*/
void hidHandleInternalSetReportResponse(HID *hid, HID_INTERNAL_SET_REPORT_RES_T *res)
{    
	hid->packet_pending = 0;    
    hidHandshakeSend(hid, res->status);
}
#endif

/****************************************************************************
NAME	
	hidHandleInternalRequestTimeout
DESCRIPTION
	This function is called when a resposne to a HID request hasn't been
    received within the timeout period.  A confirm is sent to the application
    with an timeout error status set.
RETURNS
	void
*/
void hidHandleInternalRequestTimeout(HID *hid)
{
	HID_PRINT(("hidHandleInternalRequestTimeout\n"));

    /* Tell application/host that packet has timed out */
    hidPacketFailed(hid, hid->packet_pending, hid_timeout);
}

#ifdef HID_HOST
/****************************************************************************
NAME	
	hidHandleInternalGetIdleRequest
DESCRIPTION
	This function handles the HID_INTERNAL_GET_IDLE_REQ message.
RETURNS
	void
*/
void hidHandleInternalGetIdleRequest(HID *hid, HID_INTERNAL_GET_IDLE_REQ_T *req)
{
    if (hidPacketStart(hid, HID_GET_IDLE_CFM, NULL, 0) == hidPacketStatusPending)
	{
	    /* Send HID_GET_IDLE */
		hidPacketSend(hid, (HID_GET_IDLE << 4), 0, 0);
	}
}
#endif

#ifdef HID_HOST
/****************************************************************************
NAME	
	hidHandleInternalSetIdleRequest
DESCRIPTION
	This function handles the HID_INTERNAL_SET_IDLE_REQ message.
RETURNS
	void
*/
void hidHandleInternalSetIdleRequest(HID *hid, HID_INTERNAL_SET_IDLE_REQ_T *req)
{
    if (hidPacketStart(hid, HID_SET_IDLE_CFM, NULL, 0) == hidPacketStatusPending)
	{
	    /* Send HID_SET_IDLE */
		hidPacketSend(hid, (HID_SET_IDLE << 4), &req->idle_rate, 1);
	}
}
#endif

#ifdef HID_HOST
/****************************************************************************
NAME	
	hidHandleInternalGetProtocolRequest
DESCRIPTION
	This function handles the HID_INTERNAL_GET_PROTOCOL_REQ message.
RETURNS
	void
*/
void hidHandleInternalGetProtocolRequest(HID *hid, HID_INTERNAL_GET_PROTOCOL_REQ_T *req)
{
    if (hidPacketStart(hid, HID_GET_PROTOCOL_CFM, NULL, 0) == hidPacketStatusPending)
	{
		/* Send HID_GET_PROTOCOL */
	    hidPacketSend(hid, (HID_GET_PROTOCOL << 4), 0, 0);
	}
}
#endif

#ifdef HID_HOST
/****************************************************************************
NAME	
	hidHandleInternalSetProtocolRequest
DESCRIPTION
	This function handles the HID_INTERNAL_SET_PROTOCOL_REQ message.
RETURNS
	void
*/
void hidHandleInternalSetProtocolRequest(HID *hid, HID_INTERNAL_SET_PROTOCOL_REQ_T *req)
{
    if (hidPacketStart(hid, HID_SET_PROTOCOL_CFM, NULL, 0) == hidPacketStatusPending)
	{
		/* Send HID_SET_PROTOCOL */
		hidPacketSend(hid, (HID_SET_PROTOCOL << 4) | (req->protocol & 0x01), 0, 0);
	}
}
#endif

#ifdef HID_HOST
/****************************************************************************
NAME	
	hidHandleInternalGetReportRequest
DESCRIPTION
	This function handles the HID_INTERNAL_GET_REPORT_REQ message.
RETURNS
	void
*/
void hidHandleInternalGetReportRequest(HID *hid, HID_INTERNAL_GET_REPORT_REQ_T *req)
{
    uint8 packet[3];
    uint8 packet_size = 0;
    uint8 header;
    
    if (hidPacketStart(hid, HID_GET_REPORT_CFM, NULL, 0) == hidPacketStatusPending)
	{
	    /* Build header */
	    header = (HID_GET_REPORT << 4) | (req->report_type & 0x03);
    
	    /* Add report ID to packet if present */
	    if (req->report_id)
			packet[packet_size++] = req->report_id;

		/* Add buffer size to packet if present */
		if (req->report_length)
		{
	        packet[packet_size++] = (req->report_length >> 0) & 0xFF;
			packet[packet_size++] = (req->report_length >> 8) & 0xFF;
			header |= (1 << 3);        
		}
    
	    /* Send HID_GET_REPORT */
	    hidPacketSend(hid, header, packet, packet_size);
	}
}
#endif

#ifdef HID_HOST
/****************************************************************************
NAME	
	hidHandleInternalSetReportRequest
DESCRIPTION
	This function handles the HID_INTERNAL_SET_REPORT_REQ message.
RETURNS
	void
*/
void hidHandleInternalSetReportRequest(HID *hid, HID_INTERNAL_SET_REPORT_REQ_T *req)
{
    if (hidPacketStart(hid, HID_SET_REPORT_CFM, NULL, 0) == hidPacketStatusPending)
	{
	    /* Send HID_SET_REPORT */
		hidPacketSend(hid, (HID_SET_REPORT << 4) | (req->report_type & 0x03), req->report_data, req->report_length);
	}
}
#endif

/****************************************************************************
NAME	
	hidInterruptReport
DESCRIPTION
	This function handles the sending a report on the interrupt channel.
RETURNS
	void
*/
void HidInterruptReport(HID *hid, hid_report_type report_type, uint16 report_length, const uint8 *data)
{
	/* Get interrupt sink */
	Sink sink = hid->connection[HID_CON_INTERRUPT].con.sink;
	
	/* Get pointer to stream */
	uint8 *ptr = SinkMap(sink);

	uint16 offset = SinkClaim(sink, report_length + 1);
	if (offset == 0xFFFF)
		return;

	/* Store header */
	ptr[offset] = (HID_DATA << 4) | (report_type & 0x03);

	/* Copy data into packet */
	memcpy(&ptr[offset + 1], data, report_length);

	/* Flush packet */
	SinkFlush(sink, report_length + 1);
}

#ifdef HID_HOST
void HidGetReport(HID *hid, hid_report_type report_type, uint8 report_id, uint16 report_length)
{
    MAKE_HID_MESSAGE(HID_INTERNAL_GET_REPORT_REQ);
    message->hid = hid;
    message->report_type = report_type;
    message->report_id = report_id;
    message->report_length = report_length;
	MessageSend(&hid->task, HID_INTERNAL_GET_REPORT_REQ, message);
}
#endif

#ifdef HID_HOST
void HidSetReport(HID *hid, hid_report_type report_type, uint16 report_length, const uint8 *data)
{
    MAKE_HID_MESSAGE_WITH_LEN(HID_INTERNAL_SET_REPORT_REQ, report_length);
    message->hid = hid;
    message->report_type = report_type;
    message->report_length = report_length;
	memcpy(message->report_data, data, report_length);
	MessageSend(&hid->task, HID_INTERNAL_SET_REPORT_REQ, message);
}
#endif

#ifdef HID_HOST
void HidGetIdle(HID *hid)
{
    MAKE_HID_MESSAGE(HID_INTERNAL_GET_IDLE_REQ);
    message->hid = hid;
	MessageSend(&hid->task, HID_INTERNAL_GET_IDLE_REQ, message);
}
#endif

#ifdef HID_HOST
void HidSetIdle(HID *hid, uint8 idle_rate)
{
    MAKE_HID_MESSAGE(HID_INTERNAL_SET_IDLE_REQ);
    message->hid = hid;
    message->idle_rate = idle_rate;
	MessageSend(&hid->task, HID_INTERNAL_SET_IDLE_REQ, message);
}
#endif

#ifdef HID_HOST
void HidGetProtocol(HID *hid)
{
    MAKE_HID_MESSAGE(HID_INTERNAL_GET_PROTOCOL_REQ);
    message->hid = hid;
	MessageSend(&hid->task, HID_INTERNAL_GET_PROTOCOL_REQ, message);
}
#endif

#ifdef HID_HOST
void HidSetProtocol(HID *hid, hid_protocol protocol)
{
    MAKE_HID_MESSAGE(HID_INTERNAL_SET_PROTOCOL_REQ);
    message->hid = hid;
    message->protocol = protocol;
	MessageSend(&hid->task, HID_INTERNAL_SET_PROTOCOL_REQ, message);
}
#endif

void HidControl(HID *hid, hid_control_op op)
{
	MAKE_HID_MESSAGE(HID_INTERNAL_CONTROL_REQ);
	message->hid = hid;
	message->op = op;
	MessageSend(&hid->task, HID_INTERNAL_CONTROL_REQ, message);
}

#ifdef HID_DEVICE
void HidGetReportResponse(HID *hid, hid_status status, hid_report_type report_type, uint16 report_length, const uint8 *data)
{
    MAKE_HID_MESSAGE_WITH_LEN(HID_INTERNAL_GET_REPORT_RES, report_length);
	HID_PRINT(("HidGetReportResponse\n"));
    message->hid = hid;
    message->status = status;
    message->report_type = report_type;
    message->report_length = report_length;
	memcpy(message->report_data, data, report_length);
	MessageSend(&hid->task, HID_INTERNAL_GET_REPORT_RES, message);
}
#endif

#ifdef HID_DEVICE
void HidSetReportResponse(HID *hid, hid_status status)
{
    MAKE_HID_MESSAGE(HID_INTERNAL_SET_REPORT_RES);
	HID_PRINT(("HidSetReportResponse\n"));
    message->hid = hid;
    message->status = status;
	MessageSend(&hid->task, HID_INTERNAL_SET_REPORT_RES, message);
}
#endif

#ifdef HID_DEVICE
void HidGetIdleResponse(HID *hid, hid_status status, uint8 idle_rate)
{
    MAKE_HID_MESSAGE(HID_INTERNAL_GET_IDLE_RES);
    message->hid = hid;
    message->status = status;
    message->idle_rate = idle_rate;
	MessageSend(&hid->task, HID_INTERNAL_GET_IDLE_RES, message);
}
#endif

#ifdef HID_DEVICE
void HidSetIdleResponse(HID *hid, hid_status status)
{
    MAKE_HID_MESSAGE(HID_INTERNAL_SET_IDLE_RES);
    message->hid = hid;
    message->status = status;
	MessageSend(&hid->task, HID_INTERNAL_SET_IDLE_RES, message);
}
#endif

#ifdef HID_DEVICE
void HidGetProtocolResponse(HID *hid, hid_status status, hid_protocol protocol)
{
    MAKE_HID_MESSAGE(HID_INTERNAL_GET_PROTOCOL_RES);
    message->hid = hid;
    message->status = status;
    message->protocol = protocol;
	MessageSend(&hid->task, HID_INTERNAL_GET_PROTOCOL_RES, message);
}
#endif

#ifdef HID_DEVICE
void HidSetProtocolResponse(HID *hid, hid_status status)
{
    MAKE_HID_MESSAGE(HID_INTERNAL_SET_PROTOCOL_RES);
    message->hid = hid;
    message->status = status;
	MessageSend(&hid->task, HID_INTERNAL_SET_PROTOCOL_RES, message);
}
#endif
