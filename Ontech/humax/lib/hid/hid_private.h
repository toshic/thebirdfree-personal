...
#define MAKE_HID_MESSAGE_OR_NULL(TYPE) \
	TYPE##_T *message = malloc(sizeof(TYPE##_T));
...
#endif
#ifdef HID_HOST
	HID_INTERNAL_GET_IDLE_REQ,
    HID_INTERNAL_SET_IDLE_REQ,
    HID_INTERNAL_GET_PROTOCOL_REQ,
    HID_INTERNAL_SET_PROTOCOL_REQ,
    HID_INTERNAL_GET_REPORT_REQ,
    HID_INTERNAL_SET_REPORT_REQ,
#endif
...
typedef struct
{
	Task app_task;
	bdaddr bd_addr;
    const hid_connection_config *config;
...
{
    HID *hid;
} HID_INTERNAL_GET_IDLE_REQ_T;

typedef struct
{
    HID *hid;
    uint8 idle_rate;
} HID_INTERNAL_SET_IDLE_REQ_T;

typedef struct
{
    HID *hid;
} HID_INTERNAL_GET_PROTOCOL_REQ_T;

typedef struct
{
    HID *hid;
    hid_protocol protocol;
} HID_INTERNAL_SET_PROTOCOL_REQ_T;
