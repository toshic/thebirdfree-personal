...
{	
	if(version != bluetooth_unknown)
	{
    	...
    	memmove(prim->link_key, p_dev_record->link_key, SIZE_LINK_KEY);
										...
										uint16 bonded)
{
	...
	memmove(record.link_key, link_key, SIZE_LINK_KEY);
...
uint16 connectionAuthInit(cl_dm_bt_version version)
{
	...
	else
	{
		...
		for(rec = 0; (tdi.order[rec]!=0) && (rec < NO_DEVICES_TO_MANAGE); rec++)
		{
			/* For all devices in the list... */
			if(PsRetrieve(TRUSTED_DEVICE_LIST_BASE + tdi.order[rec] - 1, &record, sizeof(TrustedDeviceRecordType)))
			{
...
uint16 connectionAuthGetDevice(const bdaddr *peer_bd_addr, cl_sm_link_key_type *link_key_type, uint8 *link_key, uint16 *trusted)
{
	...
	if(rec)
	{
		...
		*link_key_type = record.link_key_type;
		memmove(link_key, record.link_key, SIZE_LINK_KEY);
	}
...
void connectionAuthSendLinkKey(const bdaddr* peer_bd_addr)
{
	...
	if(rec)
	{
		...
		prim->valid = 1;
		memmove(prim->key, record.link_key, SIZE_LINK_KEY);
	}
	else
	{
...
RETURNS
*/
bool connectionAuthGetAttributeNow(uint16 ps_base, const bdaddr* bd_addr, uint16 size_psdata, uint8* psdata)
{
	/* Find device in the TDL */
	uint16 index = find_trusted_device(bd_addr);
	bool success = FALSE;
	...
	if(index)
	{
		...
		{
			/* Read attribute data */
			if(PsRetrieve(ps_base + index, psdata, size_psdata))
			{
				success = TRUE;
			}
		}
...
RETURNS
*/
bool connectionAuthGetIndexedAttributeNow(uint16 ps_base, uint16 mru_index, uint16 size_psdata, uint8 *psdata, bdaddr *addr)
{
	TrustedDeviceIndexType		tdi;
    TrustedDeviceRecordType     record;
	bool success = FALSE;
	...
	if ((mru_index < MAX_TRUSTED_DEVICES) && PsRetrieve(TRUSTED_DEVICE_INDEX, &tdi, sizeof(tdi)))
	{
		...
		if (tdi.order[mru_index] && PsRetrieve((TRUSTED_DEVICE_LIST_BASE + tdi.order[mru_index] - 1), &record, sizeof(TrustedDeviceRecordType)))
		{
            ...
            *addr = record.bd_addr;
			...
			{
				/* Read attribute data */
				if(PsRetrieve(ps_base + tdi.order[mru_index]-1, psdata, size_psdata))
					success = TRUE;
			}
			else
			{
