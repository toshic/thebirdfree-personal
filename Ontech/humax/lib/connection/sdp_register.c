...
void ConnectionRegisterServiceRecord(Task appTask, uint16 num_rec_bytes, const uint8 *service_record)
{
...

	{
		...
		MessageSendConditionallyOnTask(connectionGetCmTask(), CL_INTERNAL_SDP_REGISTER_RECORD_REQ, message, &theCm.sdpState.sdpLock);
	}
}
...
void ConnectionUnregisterServiceRecord(Task appTask, uint32 service_record_hdl)
{
	...
	MessageSendConditionallyOnTask(connectionGetCmTask(), CL_INTERNAL_SDP_UNREGISTER_RECORD_REQ, message, &theCm.sdpState.sdpLock);
}
