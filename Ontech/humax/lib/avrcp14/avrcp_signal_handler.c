...
void avrcpHandleReceivedData(AVRCP *avrcp)
{
    ...
    while (((packet_size = SourceBoundary(source)) != 0) && 
            !avrcp->block_received_data && !avrcp->srcUsed)
    {
        const uint8 *ptr = SourceMap(source);

        if(avrcpAvctpReceiveMessage( avrcp, ptr, packet_size))
        {
        ...
        {
            /* Drop the souce if it was not dropped */    
                            ...
                            uint16 data)
{
    ...
    MessageSendLater(&avrcp->task, AVRCP_INTERNAL_SEND_RESPONSE_TIMEOUT,
                     message, AVCTP_SEND_RESPONSE_TIMEOUT);
}
        ...
        const AVRCP_INTERNAL_SEND_RESPONSE_TIMEOUT_T *res)
{
    ...
    case avrcp_vendor:
        ...
        break;
    case avrcp_get_caps:
        avrcpSendGetCapsResponse(avrcp, avctp_response_rejected,
                                 res->data, 0, 0);
        break;
    ...
    case avrcp_player_setting_changed:  /* fall through */
        ...
        clearRegisterNotification(avrcp, 
                    (res->pending_command - avrcp_events_start_dummy)); 
        break;
    ...
    /* For the above commands, lib is posting internal 
        messages to send response, which is not necessery since it may cause 
        a race with the application response as well as an unnecessery message 
        posting. logic is changed for the below commands and above logic also 
    ...
    default:                       /* fall through */
        /* Application has not responded yet. Free the source now. */    
        avrcpSourceProcessed(avrcp,TRUE);
