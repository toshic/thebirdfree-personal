...
void a2dpHandleWatchdogInd(A2DP *a2dp)
{
			...
			(a2dp->signal_conn.signalling_state == avdtp_state_remote_aborting))
	{
		...
		a2dpResetSignalling(a2dp, FALSE);
	}
	else
	{
		/* Abort */
		a2dpAbortSignalling(a2dp, FALSE);
	}
}


/*****************************************************************************/
void a2dpAbortSignalling(A2DP *a2dp, bool link_loss)
{
...
/*****************************************************************************/
void a2dpResetSignalling(A2DP *a2dp, bool link_loss)
{
		...
		case avdtp_state_local_opening:
		{
			...
			*/
			if (a2dp->signal_conn.connect_then_open_media)
				a2dpSendConnectOpenCfm(a2dp, a2dp->clientTask, a2dp_operation_fail);
			else
				a2dpSendOpenCfm(a2dp, a2dp->clientTask, a2dp_operation_fail);
		}
		break;
