...
//    Timer2 = 50;    /* Wait for ready in timeout of 500ms */
    ...
    do
        res = rcvr_spi();
    while ((res != 0xFF) && (xTaskGetTickCount() - tick < 500 / portTICK_RATE_MS));
...
//    Timer1 = 10;
    do {                            /* Wait for data packet in timeout of 100ms */
        token = rcvr_spi();
    } while ((token == 0xFF) && (xTaskGetTickCount() - tick < 100 / portTICK_RATE_MS));
...
//        Timer1 = 100;                        /* Initialization timeout of 1000 msec */
        ...
        if (send_cmd(CMD8, 0x1AA) == 1) {    /* SDC Ver2+ */
            ...
            if (ocr[2] == 0x01 && ocr[3] == 0xAA) {    /* The card can work at vdd range of 2.7-3.6V */
                ...
                } while (xTaskGetTickCount() - tick < 1000 / portTICK_RATE_MS);
                if ((xTaskGetTickCount() - tick < 1000 / portTICK_RATE_MS) && send_cmd(CMD58, 0) == 0) {    /* Check CCS bit */
                    for (n = 0; n < 4; n++) ocr[n] = rcvr_spi();
                    ty = (ocr[0] & 0x40) ? 6 : 2;
                }
            }
            ...
            if (!(xTaskGetTickCount() - tick < 1000 / portTICK_RATE_MS) || send_cmd(CMD16, 512) != 0)    /* Select R/W block length */
                ty = 0;
        }
    }
    CardType = ty;
...
DWORD get_fattime (void)
{
    ...
    return    ((timeinfo->tm_year + 80) << 25)    // Year = 2007
            ...
            | (timeinfo->tm_min << 5)            // Min = 38
            | (timeinfo->tm_sec >> 1)                // Sec = 0
            ;
